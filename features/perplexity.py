...
class PerplexityFeature(FeatureGenerator):
    def __init__(self, device, local_device, fixed_length, experiment, models):
        self.device = device
    ...
    def features(self, documents):
        ...
        results = np.zeros((len(documents), len(self.models)))
            ...
            with torch.no_grad():
                for i_b, batch in enumerate(batches):
                    ...
                    for i in range(len(batch)):
                        results[i_b * self.batch_size + i, i_m] = ppl
