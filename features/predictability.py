...
class PredictabilityFeature(FeatureGenerator):
    def __init__(
        self, device, local_device, language, batch_size, fixed_length, experiment
    ):
        self.device = device
    ...
    def features(self, sentences):
        print("Computing PredictabilityFeature")
        FEATURE_NUM = 3
        results = np.zeros(
            (len(sentences), self.fixed_length, FEATURE_NUM * len(self.models) + 1)
        )
        ...
        for i_m, model_id in enumerate(self.models):
            ...
            with torch.no_grad():
                for i_b, batch in enumerate(batches):
                    ...
                    if i_m == 0:
                        reference_tokenisations[i_b] = encodings[
                            "offset_mapping"
                        ].numpy()
                    ...
                    probs_seen = torch.gather(
                        probs, 2, shift_labels.reshape(shift_labels.shape + (1,))
                    ).reshape(shift_labels.shape)
                    greedy = torch.argmax(probs, -1)
                    probs_greedy = torch.gather(
                        probs, 2, greedy.reshape(greedy.shape + (1,))
                    ...
                    log_probs_seen = np.concatenate(
                        (np.array([[0]] * len(batch)), log_probs_seen), axis=1
                    )
                    log_probs_greedy = (
                        torch.log(probs_greedy).to(self.local_device).numpy()
                    )
                    log_probs_greedy = np.concatenate(
                        (np.array([[0]] * len(batch)), log_probs_greedy), axis=1
                    )
                    entropy = (
                        torch.sum(torch.log2(probs + eps) * (-probs), dim=-1)
                        .to(self.local_device)
                        .numpy()
                    )
                    entropy = np.concatenate(
                        (np.array([[0]] * len(batch)), entropy), axis=1
                    )
                    # Store the mask data
                    mask = np.array(
                        [
                            x.to(self.local_device).numpy()
                            for x in encodings["attention_mask"]
                        ]
                    )
                    if i_m == 0:
                        results[
                            (i_b * self.batch_size) : (
                                i_b * self.batch_size + len(batch)
                            ),
                            : mask.shape[1],
                            0,
                        ] = mask
                    ...
                    results[
                        (i_b * self.batch_size) : (i_b * self.batch_size + len(batch)),
                        : mask.shape[1],
                        i_m * FEATURE_NUM + 1,
                    ] = (
                        log_probs_seen * mask
                    ...
                    for i in range(len(batch)):
                        aggregated_results[
                            i_b * self.batch_size + i, i_m * FEATURE_NUM
                        ] = np.mean(log_probs_seen[i, mask[i] == 1])
                        aggregated_results[
                            i_b * self.batch_size + i, i_m * FEATURE_NUM + 1
                        ] = np.mean(log_probs_greedy[i, mask[i] == 1])
                        aggregated_results[
                            i_b * self.batch_size + i, i_m * FEATURE_NUM + 2
                        ] = np.mean(entropy[i, mask[i] == 1])
